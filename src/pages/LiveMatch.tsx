...
// Bot pool for variety
const BOT_POOL = [
  { name: "StockfishBot", difficulty: "hard", style: "Aggressive" },
  { name: "AlphaClone", difficulty: "hard", style: "Positional" },
  { name: "DeepPawn", difficulty: "medium", style: "Tactical" },
  { name: "KnightRider", difficulty: "medium", style: "Dynamic" },
  { name: "RookieBot", difficulty: "easy", style: "Defensive" },
  { name: "BishopAI", difficulty: "easy", style: "Solid" },
  { name: "QueenGambit", difficulty: "hard", style: "Sacrificial" },
  { name: "PawnStorm", difficulty: "medium", style: "Attacking" },
];
...
export default function LiveMatch() {
  ...
  const [matchup, setMatchup] = useState(getRandomMatchup());
  ...
  useEffect(() => {
      ...
      if (botMove) {
        ...
        if (move) {
          ...
          if (currentGame.isGameOver()) {
            ...
            setShowGameOver(true);
            const duration = Math.floor((Date.now() - startTime) / 1000);
            saveCompletedGame(matchup.bot1.name, matchup.bot2.name, currentGame.isCheckmate() ? (currentGame.turn() === "w" ? "black" : "white") : "draw", newState.moveHistory, duration);
            toast.success("Match completed! Saved to history.");
          }
        }
      }
    ...
    return () => clearInterval(interval);
  }, [matchStatus, ttsEnabled, matchup]);
  ...
  const startNewMatch = () => {
    ...
    setMatchup(getRandomMatchup()); // Get new random matchup
    stop();
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-[#5C4A3A] text-sm">{matchup.bot1.name} vs {matchup.bot2.name}</p>
          </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#8B7A6A]">White • {matchup.bot1.style} • {matchup.bot1.difficulty}</div>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#8B7A6A]">Black • {matchup.bot2.style} • {matchup.bot2.difficulty}</div>
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div className="neu-raised rounded-3xl p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => { setTtsEnabled(!ttsEnabled); if (!ttsEnabled) stop(); }} className={`text-xs px-3 py-1 rounded-lg transition-all ${ttsEnabled ? "bg-[#8B6914] text-white" : "neu-btn text-[#5C4A3A]"}`}>
                  {ttsEnabled ? "🔊 On" : "🔇 Off"}
                </button>
              </div>
            ...
            {matchStatus === "completed" && (
              <button onClick={startNewMatch} className="w-full py-3 bg-gradient-to-br from-[#8B6914] to-[#B8941C] text-white rounded-xl font-semibold inline-flex items-center justify-center gap-2">
                <Icon path={iconPaths.refresh} size={18} />
                New Match
              </button>
            )}
          </div>
